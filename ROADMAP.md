...
_Last updated: 2026-07-09 (LP Fund-Report Portal shipped — WS16, WS17, WS18, WS19; expired set-password link recovery planned — Part 9, WS21–WS22)_
...
> **Next up — Portfolio Ledger, Cross-Fund Views, Derived Metrics & Sheet Ingestion (Part 10 of [`docs/IMPLEMENTATION_PLAN.md`](./docs/IMPLEMENTATION_PLAN.md), WS23–WS27; planned 2026-07-15 from the user's LP-platform/portfolio-tracker PRD; decisions Q21–Q27 all answered 2026-07-15).** Evolves the deal tracker from snapshot-style rows into a ledger: financing rounds, valuation-mark history, fund cashflows (additive schema + a reversible backfill of the 76 production deals), admin cross-fund portfolio views, and code-computed IRR/TVPI/dilution-aware values (admin-only in v1 — nothing on `/lp` changes). Also ships a 7-day LP-session idle timeout (Q21; the 30-day persistent session stays) and, last and env-gated, a **one-way** Google-Sheets→Molly weekly sync with a diff/run history surface and a staleness banner + "Sync now" on report publish (Q22-B/Q26/Q27) — sheet-owned deal fields go read-only in admin while sync is enabled; deploys without Google credentials are unaffected. Frozen report snapshots and LP hover-card semantics are explicitly out of bounds. The PRD's "Phase 4" (whether raw data entry eventually goes fully native) is deliberately unplanned.
