...
> **Next up — LP Fund-Report Portal (planned 2026-07-08, WS16–WS19).** Web-native replacement for the twice-yearly PDF fund reports to LPs: admin-managed funds/deals/valuations (one-time spreadsheet import, Molly becomes source of truth), letter-style fund reports authored in TipTap with an explicit portfolio-company mention picker, valuation markups **frozen at publication** into per-mention snapshots, and an LP surface (`/lp`, styled after www.dfs.vc) with email-OTP login and ~30-day sessions. Full plan in Part 7 of [`docs/IMPLEMENTATION_PLAN.md`](./docs/IMPLEMENTATION_PLAN.md); open product decisions Q6–Q14 there await answers. Confidentiality ground rule: valuation/LP data never enters this (public) repo — the importer reads the tracker xlsx from a local path only.
...
| **Medium-Style Draft Composer** (planned 2026-07-08, WS20) | User-requested redesign of the draft/composer pages (founder `/updates/new`, draft edit on `/updates/[id]`, the admin per-company composer, and the planned LP fund-report editor) after Medium's draft page: chromeless centered canvas, borderless display-type title, contextual bubble toolbar, thin top bar with an ambient "Saved" state, metrics/attachments relocated into a collapsed Details disclosure. Capability parity is mandatory — templates, metrics, attachments, publish confirm, and scheduling all preserved, at most one click away. Plan in Part 8 of `docs/IMPLEMENTATION_PLAN.md`; decisions Q15–Q18 open (scope, autosave, chrome level, toolbar model). | The composer is founders' most-used writing surface; a distraction-free page raises update quality and makes writing feel less like form-filling. |
