...
| **Medium-Style Draft Composer** (planned 2026-07-08, WS20 next) | User-requested redesign of the draft/composer pages (founder `/updates/new`, draft edit on `/updates/[id]`, the admin per-company composer, and the planned LP fund-report editor) after Medium's draft page: borderless display-type title, chromeless body with a slim persistent (not bubble/floating) toolbar, thin in-content header with an ambient "Saved" state, metrics/attachments relocated into a collapsed Details disclosure. The AppShell sidebar stays (Q17 = A) — this is a content-area restyle, not a full-bleed takeover. Capability parity is mandatory — templates, metrics, attachments, publish confirm, and scheduling all preserved, at most one click away. Plan in Part 8 of `docs/IMPLEMENTATION_PLAN.md`; all decisions (Q15–Q18) were answered 2026-07-08. | The composer is founders' most-used writing surface; a distraction-free page raises update quality and makes writing feel less like form-filling. |
