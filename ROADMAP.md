...
- **Approved founders excluded from pending approvals** — ~~only users awaiting password setup appear in the queue~~ **Annotated 2026-07-09 (F21, Part 9 of `docs/IMPLEMENTATION_PLAN.md`): this claim is backwards.** The queue filters `{ status: PENDING, approvalToken: null }` — i.e., it shows users awaiting an *admin decision*, and users awaiting password setup are precisely the population it *hides*. Approved-but-never-set-password accounts are currently invisible to admins everywhere (the root of a real founder support email about an expired link); WS22 adds an "Awaiting password setup" section with audit-logged resend.
...
> **Bug-fix batch planned 2026-07-09 — Expired Set-Password Link Recovery (Part 9, WS21–WS22).** A real founder hit the dead end: setup links expire after 48h with no self-serve re-request and no admin resend (stuck accounts are invisible to admins — see the F21 annotation under Authentication & Access above). WS21 adds an expired-state "email me a fresh link" flow on `/set-password` (token-keyed, neutral-response, rate-limited), extends the token TTL to 7 days via a single shared helper, and closes a reject-after-approve token hole (F20); WS22 adds an admin "Awaiting password setup" queue with audit-logged resend. Full plan in Part 9 of [`docs/IMPLEMENTATION_PLAN.md`](./docs/IMPLEMENTATION_PLAN.md); decisions Q19–Q20 await user sign-off (recommendations: 7-day TTL, ship WS22 now).
