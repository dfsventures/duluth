...
## WS27 — Google Sheets one-way sync + staleness affordances (~2.5–3 days) — gated, ships LAST — CODE SHIPPED 2026-07-20, APPLY DISABLED PENDING INCIDENT CLEANUP (see status note below — do not lift the gate without reading it)
...
**Status (2026-07-20): schema live, all code shipped, but a live-verification incident left 27 duplicate `Deal` rows in production and a new safety gate blocking every future apply until that's cleaned up. Read this whole note before touching sync again.**
...
**A second, independent problem surfaced in the process:** `GOOGLE_SA_EMAIL`/`GOOGLE_SA_PRIVATE_KEY`/`SHEETS_SPREADSHEET_ID` are provisioned in Vercel as write-only ("Encrypted", and empirically un-pullable — `vercel env pull` returns empty strings for exactly these three, while `CRON_SECRET`/`DATABASE_URL` pull correctly). This means **`scripts/link-sheet-rows.ts`, as written, cannot run locally at all** — the house `vercel env pull` procedure that every prior script in this project relied on doesn't work for Sheets credentials specifically. The one-time linking step needs to be redesigned as a server-side flow (an admin-gated API route, run inside Vercel where the real env vars resolve at runtime) before it can ever execute, dry-run or otherwise. `scripts/link-sheet-rows.ts` is shipped as-is (its matching logic is sound and reusable) but is **not currently invokable** — flagging this rather than deleting it, since porting its logic into a route is mechanical.

**Two explicit, separate things need a human decision before WS27 can be considered done:**
1. **Delete the 27 duplicate rows.** They are exactly and only `Deal` rows where `sheetRowId IS NOT NULL` (zero legitimate deals have this set yet) — `db.deal.deleteMany({ where: { sheetRowId: { not: null } } })` restores prod to its pre-incident state precisely. This is a destructive production action and was correctly *not* executed without a fresh, specific authorization per this project's ground rule — it's called out here for whoever picks this up next.
2. **Redesign the linking step** as an admin-route flow (reusing `scripts/link-sheet-rows.ts`'s matching function), run its dry-run live, get a clean report (0 ambiguous/unmatched), then `--apply` — only after which `SHEETS_SYNC_APPLY_ENABLED=true` should be set to re-enable real syncs.

Until both of those happen, WS27's cron/manual-sync machinery is intentionally inert (dry-run only) by design, and that's the correct, safe state — not a regression.
...
`.env.example` has the three names (values stay Vercel-only). `ROADMAP.md` updated per the Part 10 bookkeeping note below. **Not done:** the `SETUP.md` "Google Sheets ingestion" section — deferred until the linking-step redesign (see status note) is settled, so the doc describes the actual final setup flow rather than one that gets rewritten again immediately after.
...
- [x] **Fork story (ground rule 4):** with the three env vars absent, `sheetsSyncEnabled()` is false everywhere it's checked (cron no-ops 200, `/admin/sync` nav/page hidden, publish banner hidden, deals fully editable) — code-reviewed and consistent with the shipped route-access/sidebar/deal-route logic; not separately re-verified on a creds-less preview deploy this session
...
- [x] `/api/admin/sheets-sync` and `/admin/sync` both 307-redirect with no session — live-verified
- [ ] Live sync end-to-end (dry-run → apply → `ValuationMark(source:"SHEET")` → `/admin/audit` → draft-vs-published snapshot proof) — **blocked**: apply is intentionally disabled (`SHEETS_SYNC_APPLY_ENABLED`) pending the duplicate-row cleanup and the linking-step redesign; see status note
- [ ] Synced deal PATCH on a sheet-owned field → 409; `ownershipPct` PATCH → 200; manually-created deal keeps full CRUD — code-reviewed, not live-exercised against a legitimately-synced deal (none exist yet — only the 27 incident duplicates, which the fix shouldn't be verified against)
